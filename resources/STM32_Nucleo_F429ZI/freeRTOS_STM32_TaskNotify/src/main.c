...
int main(void)
{
	...
	xTaskCreate(led_task_handler, "LED-TASK", 1024, NULL, 2,  &xTaskHandle1);
	xTaskCreate(button_task_handler, "BUTTON-TASK", 1024, NULL, 2,  &xTaskHandle2);
...
void led_task_handler(void *params)
{
	uint32_t notificaton_value;
	while(1)
	{
		// wait until we get notification from button task.
		if( xTaskNotifyWait(0,0,&notificaton_value,portMAX_DELAY) == pdTRUE)
		{
			GPIO_ToggleBits(GPIOB, GPIO_Pin_7);
			sprintf(usr_msg, "Button pressed notification value = %ld \r\n", notificaton_value);
			printmsg(usr_msg);
		}
	}
}
...
void button_task_handler(void *params)
{
	while(1)
	{
		...
		{
		   //lets wait here for 100ms to compensate for the
			//button deboucing.
			/*Bouncing is the tendency of any two metal
			 * contacts in an electronic device to
			 * generate multiple signals as the contacts
			 * close or open; debouncing is any kind of
			 * hardware device or software that ensures
			 * that only a single signal will be acted
			 * upon for a single opening or closing of
			 * a contact.
			 */
            rtos_delay(100);
            // increment task notification value
