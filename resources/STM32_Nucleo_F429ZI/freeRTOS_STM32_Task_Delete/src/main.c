...
int main(void)
{
	...
	xTaskCreate(vTask1_handler, "TASK-1", 1024, NULL, 1,  &xTaskHandle1);
	xTaskCreate(vTask2_handler, "TASK-2", 1024, NULL, 4,  &xTaskHandle2);
...
void vTask1_handler(void *params)
{
	while(1)
	{
		//vTaskDelay allows the idle task to run it is a blocking call.
		// with vTaskDelay - the task2 marked deletion will be deleted by the idle task.
		// with vTaskDelay the task1 handler will be put in the block state
		// when it is in the block state the other tasks will be able to run.
		// in our case idle task will run and idle task will free the memory TCB-TASK2 memory in our example.
		vTaskDelay(200);
        printmsg("Task 1 -- toggling the LED now \r\n");
...
void vTask2_handler(void *params)
{
	while(1)
	{
		if ( GPIO_ReadInputDataBit(GPIOC, GPIO_Pin_13) == FALSE)
		{
		    GPIO_ToggleBits(GPIOB, GPIO_Pin_7);
		    rtos_delay(1000);
		}
		...
		{
			printmsg("Task Deleted\r\n");
			vTaskDelete(NULL);
			//delete itself -therefore we are passing NULL
		}
	}
}
